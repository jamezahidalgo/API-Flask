...
class TareaModel():
  ...
  @classmethod
  def getPendientes(self, user):
    try:
        cx = get_connection()
        tareas = []
        with cx.cursor() as cursor:
          cursor.execute('SELECT numero_tarea, descripcion, finalizada FROM tarea WHERE email_user = %s AND NOT finalizada', (user,))
          resultset = cursor.fetchall()
          for row in resultset:
            tarea = Tarea(row[0], user, row[1], row[2])
            tareas.append(tarea.to_JSON())
  ...
  def add_tarea(self, tarea):
    try:
      cx = get_connection()
      with cx.cursor() as cursor:
          cursor.execute("INSERT INTO tarea(email_user, descripcion, finalizada) VALUES(%s, %s, %s )", (tarea.usuario, tarea.descripcion, False))
  ...
  def finish_task(self, tarea):
    try:
      cx = get_connection()
      with cx.cursor() as cursor:
          cursor.execute("UPDATE tarea SET finalizada = True WHERE email_user = %s AND numero_tarea = %s", (tarea.usuario, tarea.numero))
          affected_rows = cursor.rowcount
          cx.commit()
      cx.close()
      return affected_rows
